...
export default function RoomatchIcon({isAnimated}: {isAnimated?: boolean}) {
    return (
        <a href='#' className={isAnimated ? 'logo-container--animated' : 'logo-container--no-animated'}>
            <div className='logo__logo-container'>
                <img className={isAnimated ? 'logo__svg--animated' : 'logo__svg--no-animated'} src={logo} alt="logo de Roomatch" />
                <p className='logo__rommatch-text'>roomatch</p>
            </div>
